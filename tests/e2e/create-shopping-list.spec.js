  ...
  test('should not produce 400 errors when creating list with valid UUID', async ({ page }) => {
    ...
    if (await titleField.count() > 0 && await dateField.count() > 0) {
      ...
      if (await submitButton.count() > 0) {
        ...
        const uuidErrors = badResponses.filter(r => 
          r.url().includes('create-shopping-list')
        );
...
test.describe('Create Shopping List - Item Management', () => {
  ...
  test('should allow adding items to the list', async ({ page }) => {
    ...
    const addItemButton = page.locator('button:has-text("Adicionar Item"), button:has-text("Add Item"), [data-action="add-item"]').first();
    ...
    if (await addItemButton.count() > 0) {
      ...
      const productField = page.locator('input[name*="product"], input[placeholder*="Produto"]').first();
